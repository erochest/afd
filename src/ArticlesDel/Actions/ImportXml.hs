...
importXml dbFile inputFile = do
    s <- runSqlite dbFile $ do
        -- now <- liftIO getCurrentTime
        runMigration migrateAll
        walkXml inputFile
    when (_sState s /= SMPage) $
...
walkXml inputFile =
    ...
    where
        ...
        start = S SMStart M.empty Nothing Nothing Nothing

step :: MonadIO m => State -> Event -> ImportXmlT m State

step s@(S SMStart _ _ _ _) (EventBeginElement "page" _) =
    return $ s & sState .~ SMPage
               & sPage .~ Just (PInfo Nothing Nothing Nothing Nothing)
step s@(S SMStart _ _ _ _) _ = return s

step s@(S SMPage _ _ _ _) (EventEndElement "page") = do
    let page = s ^. sPage
        rev  = s ^. sRevision
    now <- liftIO getCurrentTime
    void . runMaybeT $ do
        ...
        aId   <- lift . insert $ Article pId title Nothing Nothing Nothing
                                    Nothing Nothing Nothing now Published Nothing
        void . lift . insert $ Revision rId Nothing date aId Nothing
                                    (view rComment =<< rev)
    return $ s & sState .~ SMStart
               & sPage  .~ Nothing
step s@(S SMPage _ _ _ _) (EventBeginElement "id" _) =
    return $ s & sState .~ getId SMPage (sPage . _Just . pWikiId)
step s@(S SMPage _ _ _ _) (EventBeginElement "title" _) =
    return $ s & sState .~ SMText "title" Seq.empty SMPage
                            (set (sPage . _Just . pTitle) . Just)
step s@(S SMPage _ _ _ _) (EventBeginElement "redirect" _) =
    return $ s & sState .~ SMStart
               & sPage  .~ Nothing
step s@(S SMPage _ _ _ _) (EventBeginElement "revision" _) =
    return $ s & sState    .~ SMRevision
               & sRevision .~ Just (RInfo Nothing Nothing Nothing Nothing)
step s@(S SMPage _ _ _ _) _ =
    return s

step s@(S SMRevision _ _ _ _) (EventEndElement "revision") =
    return $ s & sState .~ SMPage
               & sPage . _Just . pRevision .~ (s ^. sRevision)
               & sRevision .~ Nothing
step s@(S SMRevision _ _ _ _) (EventBeginElement "contributor" _) =
    return $ s & sState .~ SMContributor
               & sContributor .~ Just (CInfo Nothing Nothing)
step s@(S SMRevision _ _ _ _) (EventBeginElement "id" _) =
    return $ s & sState .~ getId SMRevision (sRevision . _Just . rWikiId)
step s@(S SMRevision _ _ _ _) (EventBeginElement "timestamp" _) =
    return $ s & sState .~ SMText "timestamp" Seq.empty SMRevision
                                ( set (sRevision . _Just . rDate)
                                . parseTimeM True defaultTimeLocale "%FT%TZ"
                                . T.unpack
                                )
step s@(S SMRevision _ _ _ _) (EventBeginElement "comment" _) =
    return $ s & sState .~ SMText "comment" Seq.empty SMRevision
                                (set (sRevision . _Just . rComment) . Just)
step s@(S SMRevision _ _ _ _) _ =
    return s

step s@(S SMContributor _ _ _ _) (EventEndElement "contributor") = do
...
step s@(S SMContributor _ _ _ _) (EventBeginElement "username" _) =
    return $ s & sState .~ SMText "username" Seq.empty SMContributor
                                (set (sContributor . _Just . cName) . Just)
step s@(S SMContributor _ _ _ _) (EventBeginElement "id" _) =
    return $ s & sState .~ getId SMContributor (sContributor . _Just . cWikiId)
step s@(S SMContributor _ _ _ _) _ =
    return s

step s@(S (SMText name buffer next f) _ _ _ _) (EventEndElement name')
    | name == name' = return . f (fold buffer) $ s & sState .~ next
    | otherwise     = return s
step s@(S SMText{} _ _ _ _) (EventContent c) =
    return $ s & over (sState . smtBuffer) (|> getContentText c)
step s@(S SMText{} _ _ _ _) _ = return s
